...
subtitle: "Unified Specification (Architecture + Duotronics Symbol Layer + v0.2 Implementable Freeze with Patches 0002–0011)"
doc_id: "OWAC-MQP-UNIFIED"
spec_version: "0.2.2-draft"
protocol_version: 1
date: "2026-02-19"
...
**This version (0.2.2-draft) incorporates the normative changes described in Patches 0002–0011** (see the companion patch backlog document) to resolve remaining ambiguities and freeze all interoperability-critical details.
...
* Generator polynomial: the standard RS(255,223) generator with root α = 0x02 and 32 parity symbols.
* Interleaver mapping: bytes are written row-wise into a 2×255 matrix (depth 2) and read column-wise. Specifically, after encoding, the two codewords `C0[0..254]` and `C1[0..254]` are transmitted as `C0[0], C1[0], C0[1], C1[1], ..., C0[254], C1[254]`.
...
* Trellis termination: after the last data bit, **K‑1 = 6 zero bits** are appended to the input stream (post‑stuffing) to force the encoder to the all‑zeros state. These tail bits are themselves encoded, producing 12 tail bits.
* Viterbi decoding at the receiver **MUST** use **hard‑decision** inputs (from the demodulated bit stream) and a traceback depth of at least **5 × K** (35 bits) to ensure reliable decisions. In case of ties in the Viterbi path metric, the decoder **MUST** choose the path with the lower state number (deterministic tie‑breaking).
...
To prevent denial‑of‑service and decoder ambiguity, the receiver **MUST** enforce the following per‑profile limits:

| Profile ID | Maximum burst duration (seconds) |
|------------|----------------------------------|
| 1          | 2.0                              |
| 2          | 3.5                              |
| 3          | 2.5                              |

These limits are based on the maximum message size (Section 8.5) and the overhead of each profile. The receiver **MUST** start a timer upon detecting the opening flag; if the closing flag is not received within the maximum burst duration, the receiver **MUST** abort the frame and reject with `REJ_BURST_TOO_LONG`.

Additionally, the receiver **MUST** enforce timeouts for each pipeline stage (locking, framing, CBOR parse, auth verify) to prevent resource exhaustion. These timeouts are implementation‑specific but **SHOULD** be documented in deployment guides.

When `link_suspect=true` (due to pilot faults or clipping), the receiver **MAY** apply stricter gating, e.g., only accept `HEARTBEAT` commands unless quorum mode (Section 10.3) is satisfied.
...
* Duplicate map keys are invalid and **MUST** cause rejection with `REJ_CBOR_PARSE`.
...
| 6   | `ext`     | map  | no       | Extension container (see Section 8.7)          |
...
* The receiver **MUST** reject a message if `now > exp + Δ` (strictly expired) or if `now < exp - Δ` (future timestamp, treated as invalid unless the deployment allows future‑dated commands with a bounded forward window).
...
1. Decode the received `MsgBytes` into a CBOR data model.
...
Part 2 will continue with Sections 9 through the Appendices, incorporating the remaining patch changes.
```
...
# Part 2 (Sections 9–17 + Appendices)
...
2) **Expiry (if present):** if `mid.exp` present then `now ≤ exp + Δ` (with Δ the configured skew, default 30 seconds).
...
## 10.3 Crash Consistency and Execute‑Once Guarantee

Because the receiver persists the new `(last_epoch,last_ctr)` **before** executing the command, a crash after persistence but before execution would cause the command to be permanently skipped if the sender retransmits the same `MID` (it would be classified as duplicate). To provide stronger guarantees, implementations **MAY** adopt a **two‑phase durable journal** as follows:
...
* **Phase 3 – Committed:** After successful execution, atomically replace the `PENDING` record with an `EXECUTED` record (or delete it) and update `(last_epoch,last_ctr)`.
...
If this two‑phase journal is **not** implemented, the protocol’s guarantee is “at‑most‑once, with possible permanent loss after crash” – this is acceptable for deployments where commands can be re‑issued with a higher counter.

## 10.4 Idempotence Requirements

All command types defined in Section 11 **MUST** be idempotent: executing the same command multiple times with identical parameters MUST have the same effect as executing it once. This is essential for the two‑phase journal to safely re‑drive commands after recovery.
...
* `(last_epoch,last_ctr)` storage MUST be rollback-resistant (write-ahead log + fsync at minimum; hardware monotonic counter if available).
...
owac-vectors/
  ...
  profiles/
    phy-aud-a/
      ...
      v0007_pilot_drop/
      v0008_clipping/
      v0009_profile_mismatch/       (new; profile in message != configured)
      v0010_burst_too_long/         (new; exceeds max duration)
...
* `transcript.sha256`: hash of transcript.cbor
...
* demod → framing → FCS ok → CBOR deterministic decode → recompute `TranscriptBytes` (by taking the received message and removing the `sig` field from `auth`) → signature verify ok
* replay: `(epoch=7, ctr=105) > (last_epoch,last_ctr)` → accept
* execute heartbeat: log presence; update persisted `(last_epoch,last_ctr)` to (7,105).
