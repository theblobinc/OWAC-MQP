We'll address the issues incrementally. First, here is the updated **Part 1** (Sections 1–8) of OWAC-MQP v4, incorporating the necessary fixes and enhancements. Part 2 will follow in the next response.
...
subtitle: "Unified Specification (Architecture + Duotronics Symbol Layer + v4 Implementable Freeze with Patches 0002–0011 and Correctness Fixes)"
...
**This version (0.3.0-draft) incorporates the normative changes described in Patches 0002–0011** plus the following correctness and interoperability fixes:
...
* Generator polynomial: the standard RS(255,223) generator with root α = 0x02 and 32 parity symbols. The roots used are α⁰, α¹, …, α³¹ (i.e., the first consecutive 32 powers of α).
...
6. Octetize to recover `FrameBytes`
7. Verify FCS
8. Extract `MsgBytes`
...
* Viterbi decoding at the receiver **MUST** use **hard‑decision** inputs (from the demodulated bit stream) and a traceback depth of at least **5 × K** (35 bits) to ensure reliable decisions. In case of ties in the Viterbi path metric, the decoder **MUST** choose the path with the lower state number, where the state number is the integer value of the shift‑register contents (most recent input bit as LSB). This ensures deterministic tie‑breaking.
...
To prevent denial‑of‑service and decoder resource exhaustion, the receiver **MUST** enforce the following per‑profile burst duration limits. These limits are derived from the maximum message sizes (Section 8.5) and the overhead of each profile.

| Profile ID | Maximum burst duration (seconds) | Rationale (based on MAX_MSG_BYTES=12288 for alg=2) |
|------------|----------------------------------|-----------------------------------------------------|
| 1          | 100                              | 12288 bytes × 8 × stuffing factor (~1.2) / 1200 ≈ 98 s |
| 2          | 120                              | RS overhead (~1.14) adds ~14%                       |
| 3          | 200                              | Convolutional rate 1/2 doubles bits                  |

These limits are worst‑case estimates; actual bursts may be shorter. The receiver **MUST** start a timer upon detecting the opening flag; if the closing flag is not received within the maximum burst duration, the receiver **MUST** abort the frame and reject with `REJ_BURST_TOO_LONG`.
...
* **Optional far‑future guard:** Implementations MAY also enforce an upper bound `MAX_LIFETIME` (e.g., 24 hours) to reject commands that are dated too far in the future. If this guard is enabled and `exp > now + MAX_LIFETIME`, the receiver **MUST** reject with `REJ_BAD_ARGS` (or a new code `REJ_EXP_TOO_FAR` if defined). This guard is RECOMMENDED to prevent replay attacks using very distant expiry timestamps.
...
1. Decode the received `MsgBytes` into a CBOR data model, **detecting duplicate keys** during parsing (see §8.1). If duplicates are found, reject immediately with `REJ_CBOR_PARSE`.
...
2) **Expiry (if present):** if `mid.exp` present then `now ≤ exp + Δ` (with Δ the configured skew, default 30 seconds). (See §8.3 for the complete expiry semantics, including optional far‑future guard.)

If accepted and released to execution, the receiver **MUST** atomically persist `(last_epoch, last_ctr) := (E, C)` according to one of the persistence models described in §10.3.
...
* Persist `(last_epoch,last_ctr)` atomically **before** releasing the command to the actuator.
...
* `REJ_BURST_TOO_LONG`
* `REJ_CTX_MISMATCH`
* `REJ_EXP_TOO_FAR` (optional far‑future expiry guard)
...
* `REJ_CBOR_PARSE`               (includes duplicate keys)
* `REJ_CBOR_NOT_DET`
* `REJ_CBOR_INDEFINITE`
* `REJ_SIZE_LIMIT`
...
* demod → framing → FCS ok → CBOR deterministic decode (checking for duplicate keys) → recompute `TranscriptBytes` (by taking the received message and removing the `sig` field from `auth`) → signature verify ok
* replay: `(epoch=7, ctr=105) > (last_epoch,last_ctr)` → accept
* execute heartbeat: log presence; update persisted `(last_epoch,last_ctr)` to (7,105) (using chosen persistence model).
