[Download the updated Markdown](sandbox:/mnt/data/OWAC-MQP-UNIFIED-0.3.3-draft-updated.md)

````markdown
---
title: "OWAC-MQP: One-Way Audio Control & Media Queue Protocol"
subtitle: "Unified Specification (Architecture + Duotronics Symbol Layer + Final Implementable Freeze)"
doc_id: "OWAC-MQP-UNIFIED"
spec_version: "0.3.3-draft"
protocol_version: 1
date: "2026-02-23"
...
**This version (0.3.3-draft) incorporates the normative changes described in Patches 0002–0011** plus the following correctness and interoperability fixes:
...
Deployments **MAY** additionally enable a *hard-fail* policy for clipping. If hard-fail is enabled and a clipping window overlaps a candidate frame, IMN **MUST** reject that frame with `REJ_LINK_CLIP`.
...
Deployments **MAY** additionally enable a *hard-fail* policy for pilot faults. If hard-fail is enabled and a pilot fault overlaps a candidate frame, IMN **MUST** reject that frame with `REJ_LINK_PILOT_FAULT`.
...
* If `len(FecBytes)` is not a multiple of **510 bytes** (2 × 255), the receiver **MUST** reject with `REJ_L2_FRAMING`.

5. RS de-interleave and decode to recover `BaseBytes` (or reject)
6. Extract `MsgBytes` and verify FCS using the extraction rule below
7. Release `MsgBytes` to the Message Layer
...
* If the number of decoded encoded bits is not even (not divisible by 2), the receiver **MUST** reject with `REJ_L2_FRAMING`.
...
**Definition (normative):** For purposes of this section, *frame duration* is measured from the first bit **after** the opening flag of a frame (i.e., the first bit of the stuffed frame body) until the end of the closing flag. Preamble flags occur before the opening flag and are **not** counted.

The receiver **MUST** start a timer when it consumes the opening flag and observes the first subsequent body bit. If the closing flag is not received within the maximum burst duration, the receiver **MUST** abort the frame and reject with `REJ_BURST_TOO_LONG`.
...
This check **MUST** be applied before any further processing (e.g., before transcript recomputation). It also applies recursively to any CBOR-in-bstr inside `cmd.duo` (Section 12.7). If the `duo` blob cannot be decoded as deterministic CBOR, or if its deterministic re-encoding check fails, the receiver **MUST** reject with `REJ_DUO_CANON_FAIL`.
...
* `canon_hash` (bstr, **32 bytes**) matches SHA-256 of the deterministic CBOR encoding of the canonical configuration object (as defined by the family). The exact encoding rules for the canonical object are family-specific and MUST be documented for each family.
...
`cmd[2] = duo` (when present) **MUST** be a `bstr` containing exactly one deterministic-CBOR item, called `DUO_BLOB`.

### 12.7.0 `DUO_BLOB` Envelope (Normative)

`DUO_BLOB` is a CBOR map with small unsigned integer keys and **MUST** include:

* `0`: `mode` (uint) where `0` = Mode A (direct canonical payload) and `1` = Mode B (catalog reference)

If `mode` is unknown or any required field for the selected mode is missing, the receiver **MUST** reject with `REJ_DUO_CANON_FAIL`.

Unless a Duotronics family explicitly defines an internal extension container, unknown keys inside `DUO_BLOB` **MUST** cause rejection with `REJ_DUO_CANON_FAIL` (to preserve deterministic interpretation).

The entire `DUO_BLOB` is included in `TranscriptBytes` and therefore authenticated.

### 12.7.1 Mode A: Direct Canonical Payload

In Mode A, `DUO_BLOB` **MUST** include:

* `1`: `family_id` (tstr) identifying an allow-listed Duotronics family
* `2`: `canon` (any) the family-defined canonical configuration object (must be deterministically representable)
* `3`: `desc` (map) containing sufficient descriptor fields to satisfy Section 12.5 (at minimum `canon_hash`, and any redundant checks the family requires)

The receiver MUST canonicalize as required by the family policy, recompute the descriptor tuple, and verify all required descriptor fields.

### 12.7.2 Mode B: Catalog Reference

In Mode B, `DUO_BLOB` **MUST** include:

* `1`: `family_id` (tstr)
* `2`: `canon_hash` (bstr, **32 bytes**) identifying the canonical configuration
* `3`: `desc` (map) containing minimal redundant checks (e.g., `Z_primary`, `dot_count`, `orbit_size`) as required by Section 12.5/12.6
