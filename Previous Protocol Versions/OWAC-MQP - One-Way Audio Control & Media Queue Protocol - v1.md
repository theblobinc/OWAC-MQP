...
## 6.6 Optional FEC Profiles (Normative if used)

FEC is **availability-oriented**. It **MUST NOT** replace cryptographic authentication.

### 6.6.1 OWAC-FEC-CC-1/2

* Convolutional code rate 1/2, constraint length K=7
* Generator polynomials: 171(octal), 133(octal)

### 6.6.2 OWAC-FEC-RS16I2

* Reed-Solomon (255,223), 8-bit symbols, E=16
* Interleaving depth I=2

> If FEC is enabled, it is represented by the `profile` field; no implicit negotiation exists.
...
## 8.5 Size Limits (Normative)

* Entire CBOR message (before L2): **≤ 4096 bytes**
* `cmd.args` encoded: **≤ 1024 bytes**
* Any `tstr`: **≤ 256 bytes**
* Any `bstr` except signature: **≤ 2048 bytes**

Exceeding any limit ⇒ **REJECT**.

## 8.6 Canonical Transcript Bytes (Normative)

Define:

`TranscriptBytes = DeterministicCBOR( OWAC_Message with key 4 ("auth") removed )`

Authenticity verification MUST be over TranscriptBytes (or a hash thereof, depending on algorithm).
...
* `ctx` is included in TranscriptBytes (because `auth` is removed, `ctx` must be provided elsewhere if you want it bound).
  **Normative choice:** Put domain separation context into `meta` or into `cmd.args` so it is bound.
  This spec therefore defines: `meta[0]=ctx` (see CDDL).
...
## 10.1 Core Replay Rule (Normative)

Per `(sid,kid)` pair, IMN maintains `last_ctr` in durable storage.
...
If accepted and executed, IMN MUST atomically persist `last_ctr := ctr` **before** releasing the command to the actuator layer.

## 10.2 Duplicate Handling (Normative)

Define `MID := (epoch, sid, ctr)`.

* If a message arrives with `ctr == last_ctr` and its TranscriptBytes hash matches the most recently accepted message for that `(sid,kid)`, treat as **DUPLICATE**:

  * do not re-execute
  * log `duplicate=true`
* Any message with `ctr <= last_ctr` that is not the exact duplicate case is **REPLAY** ⇒ reject.
...
* Any administrative reset MUST increment `epoch` and MUST be logged as a security event.
...
META = {
  ? 0: bstr .size (0..32) ; ctx (domain separation), bound in TranscriptBytes
}
```
