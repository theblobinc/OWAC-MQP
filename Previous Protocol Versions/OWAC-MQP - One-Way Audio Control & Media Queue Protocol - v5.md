We'll produce the updated specification in two parts. Part 1 covers the Preface through Section 8. Part 2 covers Sections 9 through the Appendices. All feedback has been incorporated, with the most critical fixes highlighted.
...
**This version (0.3.1-draft) incorporates the normative changes described in Patches 0002–0011** plus the following correctness and interoperability fixes:
...
|------------|----------------------------------|---------------------------------------------------------------|
| 1          | 110                              | 12288 bytes × 8 × stuffing factor (~1.2) / 1200 ≈ 98 s, plus 2 s for preamble/margin |
...
The receiver **MUST** start a timer upon detecting the opening flag; if the closing flag is not received within the maximum burst duration, the receiver **MUST** abort the frame and reject with `REJ_BURST_TOO_LONG`.
...
**General unknown‑key rule:** Unless explicitly stated otherwise (e.g., for the `ext` container), any unknown key appearing in a protocol‑defined map (`mid`, `cmd`, `auth`, `meta`) **MUST** cause rejection with `REJ_CBOR_PARSE`. This ensures forward compatibility is managed only through the designated extension points.
...
Per `(sid,kid)`, IMN **MUST** also retain the most recently accepted `transcript_hash` (SHA-256 of TranscriptBytes, as defined in §8.9) for `(last_epoch,last_ctr)`.
