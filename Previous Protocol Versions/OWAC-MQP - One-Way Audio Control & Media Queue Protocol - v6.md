...
subtitle: "Unified Specification (Architecture + Duotronics Symbol Layer + v5 Implementable Freeze with Final Corrections)"
doc_id: "OWAC-MQP-UNIFIED"
spec_version: "0.3.2-draft"
protocol_version: 1
date: "2026-02-22"
...
**This version (0.3.2-draft) incorporates the normative changes described in Patches 0002–0011** plus the following correctness and interoperability fixes:
...
2. Form L2 frame bytes: `FrameBytes = MsgBytes || FCS16(MsgBytes)`
...
* demod → framing → FCS ok → CBOR deterministic decode (checking for duplicate keys and indefinite-length items) → recompute `TranscriptBytes` (by taking the received message and removing the `sig` field from `auth`) → signature verify ok
