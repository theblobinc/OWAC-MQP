...
- Privacy of command *metadata* and operational intent (the audio channel is plaintext; any party able to record the signal can infer actions).
...
### 5.4.1 Alternate Pilot Profiles (Informative)

Some audio front-ends (consumer laptop jacks, inexpensive USB codecs, mixers/limiters, or long cable runs) may attenuate or distort content near 19 kHz. If OWAC-PHY-AUD-A cannot maintain stable pilot tracking in a given deployment, define a **new** PHY profile rather than changing acceptance criteria.

Recommended alternatives:

* **OWAC-PILOT-17K**: Single pilot at 17.000 kHz, −30 dBFS RMS (same tracking rules as §5.4).
* **OWAC-PILOT-DUAL**: Two pilots at 17.000 kHz and 19.000 kHz, each −33 dBFS RMS, with `link_suspect=true` asserted if either pilot violates tracking (or per profile policy if one pilot is designated “secondary”).

If an alternate pilot profile is used, the selected pilot profile ID **SHALL** be recorded in deployment documentation and test vectors, and receivers **MUST** log the pilot profile ID at startup.
...
### 6.8.1 Link‑Suspect Gating Policy (Recommended Defaults)

When `link_suspect=true` (pilot faults, clipping, or other PHY integrity indicators), the receiver **SHOULD** reduce permitted side effects to minimize the chance that marginal decoding conditions trigger an unsafe action.

Recommended gating policy:

| Condition | Default allowed command types | Notes |
|---|---|---|
| `link_suspect=false` | All allow‑listed commands | Normal operation. |
| `link_suspect=true` and **no quorum** | `HEARTBEAT` only | Receiver still logs and meters link health; does not change media/transport state. |
| `link_suspect=true` and **quorum satisfied** (Section 10.5) | All allow‑listed commands except `POLICY_EPOCH_UPDATE` and `CATALOG_UPDATE` | Keep high‑impact policy changes out of degraded‑link mode by default. Deployments MAY relax this with explicit written justification. |

Deployments **MAY** tighten these defaults further (e.g., permit only transport controls but not queue edits) but SHOULD NOT loosen them without a documented safety review.
...
## 9.6 Key Lifecycle and Rotation (Operational Guidance)

Receivers are “pinned” to an allow‑list of acceptable `kid` values, not a single immutable key. Operationally:

* **Rotation model:** maintain an **active set** `{kid_i}` on the receiver, each with an operator‑managed status: `active`, `grace`, or `revoked`.
* **Planned rotation:** during a planned rotation window, receivers SHOULD accept signatures under both the *old* and *new* `kid` values, while transmitters SHOULD preferentially use the newest `kid`.
* **Revocation:** `kid` values marked `revoked` MUST be rejected with `REJ_KID_UNKNOWN` (or a distinct local alert code) and SHOULD raise an alert.
* **Auditability:** receiver logs MUST include `kid`, decision (`accept|reject`), and (if applicable) rotation state (`active|grace|revoked`) for each processed message.
* **Safety invariant:** no “automatic enrollment.” A receiver MUST NOT learn new `kid` values from the audio channel.

If a deployment requires “break glass” rollover, it MUST be implemented via a physically controlled out‑of‑band process (e.g., console access or signed removable media), not via OWAC-MQP.
...
### 11.3.0 Safety Invariants (Normative)

For all commands:

* **Allow‑list only:** the receiver MUST reject unknown `cmd.type` and unknown keys in `cmd.args`.
* **Bounded effects:** each command MUST have explicit maximum side effects (e.g., max items enqueued/removed per command).
* **No implicit interpretation:** arguments MUST NOT be treated as URLs, file paths, shell fragments, templates, or code.
* **Idempotence:** receivers SHOULD key application‑level idempotence by `MID` (and/or `transcript_hash`) so that benign duplicates do not amplify side effects.
* **Text handling:** any text fields MUST be length‑bounded, Unicode‑normalized (if applicable), and MUST NOT enable command injection into downstream components.
...
# Appendix G: Deployment Profiles and Hardware Checklist (Informative)

This appendix is intended to turn the frozen protocol profiles into a repeatable deployment.

## G.1 Recommended Audio Chain Practices

* Prefer dedicated, fixed‑gain DAC/ADC paths over laptop headset jacks when feasible.
* Disable (or avoid) AGC, “enhancements,” noise suppression, echo cancellation, and dynamic range compression.
* Ensure adequate headroom: avoid clipping at any stage; verify with injected test tones and full‑scale payload bursts.
* Use shielded cabling and physically secure the audio path as part of the “one‑way” boundary.

## G.2 Pilot Profile Selection

If OWAC-PILOT-19K is unstable on target hardware, define a new PHY profile (e.g., OWAC-PILOT-17K or OWAC-PILOT-DUAL; see §5.4.1) and regenerate golden vectors for that profile.

## G.3 Qualification Checklist (Minimum)

* Run end‑to‑end decode from raw PCM vectors (including worst‑case clipping/noise cases) and verify deterministic accept/reject.
* Verify pilot tracking thresholds with the actual deployed hardware chain.
* Confirm receiver gating defaults for `link_suspect=true` (see §6.8.1).
* Confirm log/metric export for: pilot faults, clipping windows, frame rejects, and replay rejects.

---

# Appendix H: Implementation Self‑Audit Checklist (Informative)

A receiver implementation SHOULD be able to answer “yes” to each item below:

* Deterministic CBOR: reject indefinite lengths, reject duplicate keys, reject non‑canonical encodings; re‑encode check passes.
* Transcript binding: `TranscriptBytes` includes all authenticated fields (including `ctx`) exactly once and in frozen order.
* Cryptography: unknown `kid` rejects; signature length and algorithm profile enforcement; constant‑time verify where applicable.
* Replay state: durable `(epoch, ctr, transcript_hash)` storage; atomic update; clear crash model (A or B) documented.
* Decoder limits: burst duration and timeout caps enforced irrespective of content; bounded memory growth.
* Link health: pilot/clipping metrics and `link_suspect` annotation implemented; alerts on sustained fault rates.
* Application layer: allow‑list enforcement; bounded args; idempotence keyed by `MID`; no URL/path/shell interpretation.
